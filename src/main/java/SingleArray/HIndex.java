 ...
 * 
 * Hide Hint #1 An easy approach is to sort the array first. Hide Hint #2 What
 * are the possible values of h-index? Hide Hint #3 A faster approach is to use
 * extra space.
 * 
 * @author ezbanab
 *
...
public class HIndex {
	 ...
	 * reference corresponding to the index of the bucket, we increment the count
	 * for that bucket. The only exception is that for any paper with larger number
	 * of reference than n, we put in the n-th bucket.
	 * 
	 ...
	 * be our h-index result. The reason to scan from the end of the array is that
	 * we are looking for the greatest h-index. For example, given array
	 * [3,0,6,5,1], we have 6 buckets to contain how many papers have the
