...
public class CompareVersionNumbers {
	...
	public int compareVersion(String version1, String version2) {
		...
		int v1SpiltLength = v1Split.length;
		int v2SplitLength = v2Split.length;
		
		int max = Math.max(v1SpiltLength, v2SplitLength);
		int v1, v2;
		for(int i=0; i<max;i++) {
			v1 = (i< v1SpiltLength)? Integer.valueOf(v1Split[i]):0;
			v2 = (i< v2SplitLength)?Integer.valueOf(v2Split[i]):0;
	...
	public static void main(String[] args) {
		...
		Assert.assertEquals(-1,versionNumbers.compareVersion("7.5.2.4", "7.5.3"));
		Assert.assertEquals(-1,versionNumbers.compareVersion("0.1", "1.1"));
		Assert.assertEquals(1,versionNumbers.compareVersion("1.0.1", "1"));
		Assert.assertEquals(0,versionNumbers.compareVersion("1.01", "1.001"));
		Assert.assertEquals(0,versionNumbers.compareVersion("1.0", "1.0.0"));
	}
